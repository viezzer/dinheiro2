    ...
    return (
        <div className={styles.container}>
            {transactions ? (transactions.map((transaction, index) => (
                    <div className='w-full h-11 bg-card flex flex-row rounded-lg mb-2 justify-between sm:w-96 md:w-3/4' key={index} onClick={() => {handleDelete(transaction.id, transaction.title)}}>
